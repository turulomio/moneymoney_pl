...
def calculate_io_lazy(dt, pia,  io_rows, currency_user):
    ...
    currency_product=pia["products_currency"]
    currency_account=pia["accounts_currency"]
    
    lazy_quotes[f"{pia['products_id']}#{dt}"]=None
    lazy_factors[f"{currency_product}#{currency_account}#{dt}"]=None
    ...
    for row in io_rows:
        print(row)
        lazy_factors[f"{currency_account}#{currency_user}#{row['datetime']}"]=None
        io.append(row)
    ...
    for o in io:
        o['real_leverages']= realmultiplier(pia)
        ...
        o['gross_user']="LAZY"#o['gross_account']*a2u[o['datetime']]
        o['account2user']="LAZY"#a2u[o['datetime']]
        o['gross_user']="LAZY"#o['gross_account']*o['account2user']
        if o['shares']>=0:
            o['net_account']=o['gross_account']+o['commission_account']+o['taxes_account']
        else:
            o['net_account']=o['gross_account']-o['commission_account']-o['taxes_account']
        o['net_user']="LAZY" #o['net_account']*a2u[o['datetime']]
        o['net_investment']=o['net_account']/o['currency_conversion']
        print(o)

    for c in cur:
        c['real_leverages']= realmultiplier(pia)
#        c['investment2account_at_datetime']="LAZY"#investment2account_at_datetime
#        c['account2user_at_datetime']="LAZY"#a2u[p_at_datetime] 
#        c['account2user']="LAZY"#a2u[c['datetime']]
#        c['price_account']="LAZY"#c['price_investment']*c['investment2account']
#        c['price_user']="LAZY"#c['price_account']*c['account2user']
#        c['taxes_investment']="LAZY"#c['taxes_account']/c['investment2account']#taxes and commissions are in account currency buy we can guess them
#        c['taxes_user']="LAZY"#c['taxes_account']*c['account2user']
#        c['commissions_investment']="LAZY"#c['commissions_account']/c['investment2account']
#        c['commissions_user']="LAZY"#c['commissions_account']*c['account2user']
...
#        c['balance_investment']="LAZY"#0 if data['productstypes_id'] in (12,13) else abs(c['shares'])*quote_at_datetime*c['real_leverages']
#        c['balance_account']="LAZY"#c['balance_investment']*investment2account_at_datetime
#        c['balance_user']="LAZY"#c['balance_account']*a2u[p_at_datetime]
#        #Aquí calculo con saldo y futuros y cfd
#        if c['shares']>0:
#            c['balance_futures_investment']="LAZY"#c['shares']*quote_at_datetime*c['real_leverages']
#        else:
#            diff="LAZY"#(quote_at_datetime-c['price_investment'])*abs(c['shares'])*c['real_leverages']
#            init_balance=c['price_investment']*abs(c['shares'])*c['real_leverages']
#            c['balance_futures_investment']="LAZY"#init_balance-diff
#        c['balance_futures_account']="LAZY"#c['balance_futures_investment']*investment2account_at_datetime
#        c['balance_futures_user']="LAZY"#c['balance_futures_account']*a2u[p_at_datetime]
#        c['invested_investment']=abs(c['shares']*c['price_investment']*c['real_leverages'])
#        c['invested_account']="LAZY"#c['invested_investment']*c['investment2account']
#        c['invested_user']="LAZY"#c['invested_account']*c['account2user']
#        c['gains_gross_investment']="LAZY"#(quote_at_datetime - c['price_investment'])*c['shares']*c['real_leverages']
#        c['gains_gross_account']="LAZY"#(quote_at_datetime*c['investment2account_at_datetime'] - c['price_investment']*c['investment2account'])*c['shares']*c['real_leverages']
#        c['gains_gross_user']="LAZY"#(quote_at_datetime*c['investment2account_at_datetime']*c['account2user_at_datetime'] - c['price_investment']*c['investment2account']*c['account2user'])*c['shares']*c['real_leverages']
#        c['gains_net_investment']="LAZY"#c['gains_gross_investment'] -c['taxes_investment'] -c['commissions_investment']
#        c['gains_net_account']="LAZY"#c['gains_gross_account']-c['taxes_account']-c['commissions_account'] 
#        c['gains_net_user']="LAZY"#c['gains_gross_user']-c['taxes_user']-c['commissions_user']
...
#        h['gains_net_user']=h['gains_gross_user']-h['taxes_user']-h['commissions_user']
    ...
    return dumps({ "io": io, "io_current": cur,"io_historical":hist, "lazy_quotes":lazy_quotes, "lazy_factors": lazy_factors}, indent=4,  cls=DjangoJSONEncoder)
...
def calculate_io():
    """
        ...
        CREATE OR REPLACE FUNCTION public.investment_operations(p_investment_id integer, p_at_datetime timestamp with time zone, user_currency text, tablename text DEFAULT 'investmentsoperations'::text, simulation_account_currency text DEFAULT NULL::text, simulation_products_id integer DEFAULT NULL::integer)
